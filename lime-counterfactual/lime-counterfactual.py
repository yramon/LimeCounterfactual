...
class LimeCounterfactual(object):
    ...
    def explanation(self, instance):
        ...
        Returns:
            A tuple (explanation_set, feature_coefficient_set, 
            number_active_elements, minimum_size_explanation, minimum_size_explanation_rel,
            time_elapsed, score_predicted[0], score_new[0], difference_score_lime[0], 
            expl_lime), where:
        ...
        for j in range(len(explanation_lime)):
            if explanation_lime[j][1] >= 0:   #only the features with a zero or positive estimated importance weight are considered
                ...
                index_feature = np.argwhere(np.array(feature_names)==feature) #returns index in feature_names where == feature
        ...
        if (np.size(instance) != 0):
            ...
            k = 0
            while ((score_new[0] >= self.threshold_classifier) and (k != len(indices_features_lime)) and (tic - time.time() <= self.time_maximum)):
                number_perturbed=0
                k+=1
                perturbed_instance=instance_sparse.copy()
                j=0
                for feature in indices_features_lime[0:k]:
                    number_perturbed += 1
        ...
        return (explanation_set, feature_coefficient_set, number_active_elements, minimum_size_explanation, minimum_size_explanation_rel, time_elapsed, score_predicted[0], score_new[0], difference_scores[0], expl_lime)        